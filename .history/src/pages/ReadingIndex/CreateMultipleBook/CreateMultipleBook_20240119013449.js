...
export const CreateMultipleBook = (props) => {
  ...
  const columns = [
    ...
    {
      title: "Nhân viên",
      dataIndex: "nhanVienXem",
      key: "nhanVienXem",
      render:(record) => {
        return <p>{canBoDoc.data.GetUsers.nodes.find(user => user.id === record).normalizedUserName}</p> 
      }
    },
    {
      title: "Mã tuyến đọc",
    ...
    },
    {
      ...
      key: "kyGhiChiSoId",
      render:(record) => {   
        if(record === "Kỳ ghi Cần Thơ"){
          return <p>Kỳ ghi Cần Thơ</p>
        }else{
          return <p>{kyGhi.data.GetKyGhiChiSos.nodes.find(kg => kg.id === record)?.moTa}</p>
        }
        // return <p>{record}</p>
      }
    },
          ...
          dataSource={
            dsTuyenChuaTaoSoDoc?.length > 0
              ? dsTuyenChuaTaoSoDoc?.map((_tuyenDocChuaTaoSo, index) => ({
                  ...
                  nhanVienXem: _tuyenDocChuaTaoSo.nhanVienXem,
                  keyId: _tuyenDocChuaTaoSo.keyId,
                  tenTuyen: _tuyenDocChuaTaoSo.tenTuyen,
                  kyGhiChiSoId: _tuyenDocChuaTaoSo.kyGhiChiSoId,
                }))
              : []
          }
          columns={columns}
